...
describe('route hardening', () => {
  ...
  it('caps release analysis input before calling the AI service', async () => {
    const app = express();
    ...
    const retrospectives = Array.from({ length: 75 }, (_, index) => ({ id: `retro-${index}` }));
    ...
    expect(generateReleaseAnalysis).toHaveBeenCalledWith(expect.objectContaining({
      retrospectives: retrospectives.slice(0, 50),
