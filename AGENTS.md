...
- `AUTH_RATE_LIMIT_MAX` - Max team-create / restore-session requests allowed per IP per 15 minutes (default: `5`); raised by the Playwright config so the full e2e suite can run without hitting the production safeguard. Counted **per pod**: `express-rate-limit` runs without a shared store, so a deployment with `N` replicas admits up to `N × AUTH_RATE_LIMIT_MAX` per window. Size legitimate capacity against one pod's budget and brute-force exposure against `N ×` it; a hard cluster-wide ceiling needs the Ingress/WAF or a Redis store
