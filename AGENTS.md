...
- `INVITE_MAX_PER_TEAM_PER_HOUR` - Max invitation emails one team may send per hour (default: `200`). `/api/send-invite` authenticates with the team credential, so the meter is **per team, not per IP** — a whole office shares one egress IP and a facilitator must be able to invite a large group in one batch. It bounds how much mail a single leaked team password can push through the deployment's SMTP identity. Counted per pod (like the express-rate-limit routes), over a fixed 1-hour window, on a bounded LRU of the 500 most recent teams
