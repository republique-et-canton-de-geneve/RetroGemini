...
| `SOCKET_UPDATE_RATE` | Sustained session updates per second allowed per socket. A throttled write is healed with the authoritative state, never dropped | `0` (disabled) in code; the Kubernetes manifest ships `20` |
