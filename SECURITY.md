...
**The minimum length is 12 characters, and it is the whole policy.** Every path that *sets* a password — team creation, the team's own password change, the emailed password reset and the super-admin override — enforces the same rule from a single module (`utils/passwordPolicy.js`), which the React forms read too, so the screens and the routes cannot drift apart. There is still **no complexity, dictionary or reuse check, and no account lockout**: 12 characters is the OWASP ASVS 2.1.1 floor, and character-class rules are no longer recommended by either ASVS or NIST SP 800-63B. A breached-password check would need an external service and is deliberately out of scope for an air-gapped deployment.
